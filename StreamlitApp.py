...
# UI Widget State Tracking
if 'weight' not in st.session_state:
    st.session_state['weight'] = 30.0
if 'height' not in st.session_state:
    st.session_state['height'] = 140.0
if 'creatinine' not in st.session_state:
    st.session_state['creatinine'] = 0.62
if 'birthdate_input' not in st.session_state:
    st.session_state['birthdate_input'] = datetime.date(2015, 1, 1)
if 'model_input' not in st.session_state:
    st.session_state['model_input'] = 'Xu2026'
...
with st.sidebar.popover('Input Patient Data'):
    # Bind directly to the session state keys initialized earlier
    birthdate = st.date_input(label='Birthdate', key='birthdate_input')
    weight = st.number_input(label='Weight (kg)', key='weight')
    height = st.number_input(label='Height (cm)', key='height')
    creatinine = st.number_input(label='Creatinine', key='creatinine')

    # Add the key parameter to the selectbox
    selected_model = st.selectbox(
        label='Select PK Model',
        options=['Xu2026', 'Le2013', 'Smit2021', 'Lamarre2000'],
        key='model_input'
    )
...
if st.session_state.get('patient_data_updated', False):
    ec1, ec2 = st.sidebar.columns(2)

    # 1. Text File Export
    export_string = IOfunctions.generate_patient_file_content(
        birthdate=st.session_state['birthdate_input'],
        weight=st.session_state['weight'],
        height=st.session_state['height'],
        creatinine=st.session_state['creatinine'],
        model=st.session_state['model_input'],
        doses_df=st.session_state['doses'],
    ...
    if st.session_state.get('data_fit', False):
        if ec2.button("Generate PDF", use_container_width=True):
            import ReportGenerator

            with st.spinner("Compiling PDF Report..."):
                pdf_filename = "VanKe_Doodle_Report.pdf"
                ...
                with open(pdf_filename, "rb") as pdf_file:
                    st.sidebar.download_button(
                        ...
                        data=pdf_file,
                        file_name="VanKe_Doodle_PK_Report.pdf",
                        mime="application/pdf",
                        use_container_width=True,
                        type="primary"
                    )
    else:
        ec2.button("Generate PDF", disabled=True, use_container_width=True, help="Fit data first to generate report.")

else:
    st.sidebar.info("Input or upload patient data to enable export.")
